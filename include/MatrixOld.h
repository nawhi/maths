 ...
 */
    ...
    public:
         ...
         */
        MatrixOld<I, size>() = default;

        /*
         * Constructor designed to take matrices in
         * written form in initialiser lists in code.
         */
        MatrixOld<I, size>(std::initializer_list<I> &&elems);

        /**
         * Constructor designed to take matrices in
         * a 2D vector.
         */
        MatrixOld<I, size>(std::vector<std::vector<I>> elems);

        static MatrixOld<I, size> identity();
        ...
        friend bool operator==(const MatrixOld &lhs, const MatrixOld &rhs) {
        ...
        friend MatrixOld operator*(const MatrixOld &lhs, const MatrixOld &rhs) {
            MatrixOld<I, size> ret;
 ...
 */
    template<typename I, int size>
    MatrixOld<I, size>::MatrixOld(std::initializer_list<I> &&elems) {
 ...
 */
    template<typename I, int size>
    MatrixOld<I, size>::MatrixOld(std::vector<std::vector<I>> v) {
        assert(v.size() == size);
    ...
    template<typename I, int size>
    MatrixOld<I, size> MatrixOld<I, size>::identity() {
        MatrixOld<I, size> m;
        for (int i = 0; i < size; ++i)
            for (int j = 0; j < size; ++j)
                m.set(i, j, (i == j) ? 1 : 0);
    ...
    template<typename I, int size>
    I MatrixOld<I, size>::operator()(int row, int col) const {
        assert(rangecheck(row, col));
        return get(row, col);
    }

    template<typename I, int size>
    I MatrixOld<I, size>::get(int row, int col) const {
        assert(rangecheck(row, col));
        return elements[row + size * col];
    }

    template<typename I, int size>
    void MatrixOld<I, size>::set(int row, int col, I t) {
        assert(rangecheck(row, col));
 ...
 */
    template<typename I, int size>
    std::string MatrixOld<I, size>::str() const {
        std::stringstream ss;
        for (int i = 0; i < size; i++) {
            ss << "\n[ ";
            for (int j = 0; j < size; j++)
    ...
    template<typename I, int size>
    std::ostream &operator<<(std::ostream &os, const MatrixOld<I, size> &m) {
        os << m.str();
        return os;
    }
